...
- 当前契约版本 v5.5（技能文档 v5.5）；新增动画类型或改动动画引擎实现（含兜底/回位/划线分支）时需同步升级（见 P0-3）
...
> 校验：在仓库根目录执行 `npm run check:contract`，两端契约版本不一致会报错退出。
...
- **录屏管线 v3（2026-08-29，全屏录制为唯一路径，离屏方案弃用）**：勾录屏→点播放 → 窗口全屏（菜单栏隐藏）→ getDisplayMedia 捕获主窗口 → **canvas 等比重采样到固定输出档位**（1080P/2K/4K 可选，全 16 对齐）→ 音轨从编辑器内 `<audio>` 走 **WebAudio 管线**接入 → 播完 2s 自动停录、免弹窗直接保存到 HTML 所在目录。实测：2520×1680 屏 → 1920×1080 MP4，音轨峰值 0.7356，全自动落盘。
  **"不同分辨率/比例显示器"的解法**：把"屏幕尺寸"与"输出尺寸"**解耦**——屏幕任意分辨率/比例/DPI，输出恒为标准档位（非 16:9 屏等比缩放居中信箱），编码器约束/码率不再随屏幕漂移。踩过的坑（改这块必读）：
  1) H.264 硬编码器要求**宽 16 对齐**，2520×1680 直录报 `Video encoding failed`（1920×1080 正常）→ 不要直录原生尺寸
  2) 主窗口的音频捕获轨是**静音数据**（离屏窗口同写法有声，原因未明）→ 音频别从窗口捕获取
  3) `element.captureStream()` 在元素**未播放时**拿的轨不产出数据帧，mp4 连音轨 box 都不写 → 用 WebAudio 管线（`createMediaElementSource` 对同一元素只能调一次，destination 缓存在元素上复用；必须 `connect(destination)` 接回扬声器，否则编辑器里没声）
...
- **录屏清晰度 v3.2：MediaRecorder 码率失效根治——改用 WebCodecs 编码引擎（2026-08-31）**：用户实测 25 秒成片仅 5.4MB（≈1.75Mbps），文字发虚。Electron 探针同机码率阶梯实测结论：**Chromium 126 MediaRecorder 的 `videoBitsPerSecond` 基本失效**——H.264 请求 1M~16.7M 实际只出 0.3~4.9Mbps（VP8/VP9/H.264 × var/constant × 硬编/软编全试过），v3.1 的「封顶 48M」在这类机器上是空谈；同机 **WebCodecs `VideoEncoder`（`bitrateMode:'constant'`）足额兑现 16.7Mbps**（静/动画面都是），AAC `AudioEncoder` 可用。改动：`recorder.js` 双引擎——主路径 canvas 轨→`MediaStreamTrackProcessor`→`VideoEncoder`(H.264 High CBR) + WebAudio 轨→`AudioEncoder`(AAC 192k) + **mp4-muxer** 手工封装单一 mp4（新增依赖，纯 JS 离线）；MediaRecorder 整体降级为无 WebCodecs 环境的兜底。`stop()` 契约不变（blob/ext/durationMs/recError/firstChunkMs），会话新增 `engine` 字段；`mixInAudio` 重构为 `getAudioTrack`（返回轨+采样率+声道），**音轨只收 WebAudio 来源**（采样率钉 48000，AAC/封装配置需要确定值；captureStream 兜底轨只喂 MediaRecorder）。E2E 探针验收（本机 1080p）：h264 High 1920×1088 **16.65Mbps** + AAC 音画同步 + 30fps 满帧，对照旧产物（VP9 webm 1.75Mbps）码率约 10 倍。另踩一坑：本机 node_modules 曾残留 Electron 29（package.json 已是 ^31）→ mp4 全家 `isTypeSupported=false` 静默掉 webm 兜底——**换机/拉代码后必须 `npm install`**。
